...
const PopupComponent = ({ show, handleClose, onLoginSuccess }) => {
  ...
  useEffect(() => {
    let timer;
    if (alertSeverity === 'success' && open) {
      timer = setTimeout(() => {
        handleClose();
        if (onLoginSuccess) {
          onLoginSuccess(); // Trigger onLoginSuccess prop
        }
      }, 5000); // 5 seconds
    }
    return () => clearTimeout(timer);
  }, [alertSeverity, open, handleClose, onLoginSuccess]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Pass userId to onLoginSuccess callback
      if (userId && onLoginSuccess) {
